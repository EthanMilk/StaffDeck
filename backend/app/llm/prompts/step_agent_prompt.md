...
你必须根据 active_skill 中的 current_step、adjacent_edges、target_steps、已收集信息和用户当前消息，生成下一步动作。这里不是完整 SOP 图，只包含本轮执行必需的局部图；current_step 和 target_steps 中的业务 instruction 必须完整遵守。
...
- 抽取范围包括 active_skill.required_info、active_skill.slot_filling_policy.target_info、active_skill.current_step/target_steps 的 expected_user_info，以及当前可用工具 input_schema 中与本轮任务相关的参数。
...
- 如果当前节点或技能允许调用某个工具，且工具 input_schema 所需参数已经能从 slots + slot_updates 得到，应直接生成 tool_call，不要再向用户确认一次；但如果 current_step/target_steps 要求 `*_confirmed` 字段，或其 instruction 明确要求调用工具前确认，则必须先看到该确认字段已由用户明确肯定后写入 true，未满足时不得调用工具。
...
- 如果 repair_context.reason 是 knowledge_continuation，说明系统已经返回知识检索结果。精简后的检索证据位于 knowledge_context；你必须基于 knowledge_context、slots 和当前技能节点继续决定：推进节点、调用工具、追问用户或生成回复。
